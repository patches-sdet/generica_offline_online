from domain.abilities.definitions.definitions import AbilityGrant, JobDefinition
from domain.content_registry import register_progression_ability_grant


def compile_grants(job_def: JobDefinition) -> tuple[AbilityGrant, ...]:
    implicit_grants = tuple(
        AbilityGrant(name=ability.name, required_level=ability.required_level)
...
def register_job_grants(job_def: JobDefinition, grants: tuple[AbilityGrant, ...]) -> None:
    for grant in grants:
...
def register_compiled_job(job_def: JobDefinition, compiled_abilities) -> tuple[AbilityGrant, ...]:
